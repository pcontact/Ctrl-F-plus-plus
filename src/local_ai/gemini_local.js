// local_ai/gemini_local.js
// Standalone module for on-device Gemini (Prompt API) sessions.
// No Chrome offscreen dependencies. Designed to be imported by a router or controller.
//
// Features:
...
function estimateTokens(str) {
  return Math.ceil((str?.length || 0) / TOKEN_ESTIMATE_DIVISOR);
}
...
function trimHistoryToContextLimit(history = [], newText = "") {
  const normalized = normalizeHistory(history);
  const system = normalized.find(h => h.role === "system");
  const nonSystem = normalized.filter(h => h.role !== "system");

  if (system && estimateTokens(system.content) >= MAX_CONTEXT_TOKENS) {
    console.warn("System prompt exceeds context window.");
    return [system];
  }

  let totalTokens = estimateTokens(newText);
  const trimmed = [];
  for (let i = nonSystem.length - 1; i >= 0; i--) {
    const item = nonSystem[i];
    const cost = estimateTokens(item.content || "");
    if (totalTokens + cost > MAX_CONTEXT_TOKENS) break;
    totalTokens += cost;
    trimmed.unshift(item);
  }

  if (trimmed.length === 0 && nonSystem.length >= 2) {
    const lastPair = nonSystem.slice(-2);
    const pairTokens = lastPair.reduce((s, it) => s + estimateTokens(it.content || ""), 0);
    if (estimateTokens(newText) + pairTokens <= MAX_CONTEXT_TOKENS) {
      return system ? [system, ...lastPair] : lastPair;
    }
  }
...
function buildMessagesForPrompt(userText, history = [], includeSystem = false) {
  const normalized = normalizeHistory(history);
  const msgs = [];
  for (const h of normalized) {
    if (!includeSystem && h.role === "system") continue;
    msgs.push({ role: h.role || "user", content: h.content ?? "" });
  }
...
export async function askLocalAI({
  ...
  if(!conversationId || typeof conversationId !== "string" || !session.get(conversationId)){
